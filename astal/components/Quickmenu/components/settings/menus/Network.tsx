...
export default () => {
  ...
  return (
    <Menu name="network" action={() => wifi.scan()}>
      <box spacing={options.theme.spacing.value} className="list" vertical>
        {bind(wifi, "accessPoints").as((points) =>
          points.map((ap) => (
            <button
              className={bind(wifi, "activeAccessPoint").as((aap) =>
                aap === ap ? "active" : "",
              )}
              onClicked={() => nmClient.activate_connection_async()}
            >
